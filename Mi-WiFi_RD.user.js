...
async function rSD() {
    ...
    try {
      ...
      let cWU = S.wInstUp, cWD = S.wInstDn, wanValid = !1;
      let ws = sD?.wan || sD?.wanStatistics;

      if (ws && ('upspeed' in ws) && ('downspeed' in ws)) {
        ...
        try {
          ...
          if (xR.ok) { 
            ...
            if (('upspeed' in ws) && ('downspeed' in ws)) { cWU = (+ws.upspeed || 0) * 8; cWD = (+ws.downspeed || 0) * 8; wanValid = !0; }
          }
        } catch (err) { console.warn(err) }
      }
      n = performance.now(); // devicelist 与 status 并发且设备统计混用两者，统一以整批数据就绪时间为本轮时间
      let wanCounterValid = wanValid && ws && ('upload' in ws) && ('download' in ws);
      ...
      记总速率图(wanCounterValid ? S.dWU : cWU, wanCounterValid ? S.dWD : cWD);
      let sL = sD?.dev || [], sDByMac = Object.create(null);
      ...
      (dD.list || []).forEach(i => {
        let m = nM(i.mac || "");
        if (m) {
          let x = sDByMac[m] || null, 官方累计有效 = !!x && ('upload' in x) && ('download' in x);
          ...
          cI[m] = {
            ...
            offUp: 官方累计有效 ? (+x.upload || 0) * 8 : (S.cls[m]?.lU || 0),
            offDn: 官方累计有效 ? (+x.download || 0) * 8 : (S.cls[m]?.lD || 0),
            官方累计有效,
      ...
      if (wanValid && S.wLT === undefined) { S.wLT = n; }
      ...
      for (const [m, cC] of Object.entries(cI)) {
        ...
        if (!cS) {
          ...
          cS = S.cls[m] = {
            ...
            oU: cC.offUp, oD: cC.offDn, name: spD?.name || cC.name || m, hU: new Float64Array(64), hD: new Float64Array(64), hIdx: 0, ifc: cC.iface // 真实流量与环形缓冲
          };
        } else {
          let dU = cC.offUp - cS.lU, dD = cC.offDn - cS.lD;
          if (cS.ifc !== cC.iface) { if (CONFIG.盲漫游) cS.aR = 2; cS.ifc = cC.iface; }
          if (cC.官方累计有效 && (dU < 0 || dD < 0)) {
            if (dU < 0) { cS.uB += dU; cS.dpU = cS.lU; cS.oU += dU; }
            if (dD < 0) { cS.dB += dD; cS.dpD = cS.lD; }
            cS.aR = 3;
          } else if (cC.官方累计有效 && cS.aR === 3) {
            if (dU > 0 || dD > 0) {
          ...
          if (cC.官方累计有效) { cS.lU = cC.offUp; cS.lD = cC.offDn; }
        }
      ...
      rUI(wanCounterValid ? S.dWU : cWU, wanCounterValid ? S.dWD : cWD, cSU, cSD, cI);
